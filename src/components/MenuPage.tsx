...
export function SectionTitle({ children, className, size = 'lg' }: SectionTitleProps) {
  return (
    <h2
      className={cn(
        'font-brolimo italic text-center font-light leading-none',
        size === 'lg' ? 'mb-10' : 'mb-6',
        className,
      )}
...
export function MenuItem({ name, price, description, note, className }: MenuItemProps) {
  return (
    <div className={cn('mb-10', className)}>
      ...
      <p style={{ fontSize: '14px', lineHeight: '1.45', marginTop: '6px' }}>{description}</p>
      {note && (
        <p style={{ fontSize: '11px', marginTop: '4px', color: '#555' }}>{note}</p>
      )}
    </div>
  )
}
